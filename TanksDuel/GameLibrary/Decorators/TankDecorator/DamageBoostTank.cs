...
{
    /// <summary>
    /// Класс танка с усиленными боеприпасами
    /// </summary>
    public class DamageBoostTank : TankDecorator
    {
        private const int _boostDurationSeconds = 25;

        private readonly DateTime _endOfBoost;
        private readonly Timer _boostTimer;

        /// <summary>
        /// Конструктора класса танка с усиленными боеприпасами
        /// </summary>
        public DamageBoostTank(NormalTank tank) : base(tank)
        {
        ...
        public override void _controller_Shooting(object sender, EventArgs e)
        {
            Ammo ammo = Weapons?.Shot();
            if (ammo != null)
            {
                Ammo decoratedAmmo = new AmmoDamageDecorator(ammo, ammo.Textures);
                if (decoratedAmmo != null)
                {
                    if (GameField != null)
                    {
                        if (GameField.Shots != null)
                        {
                            GameField.Shots.Add(decoratedAmmo);
                        }
                    }
                }
             }
